...
class Git {
  ...
  async commit(msg) {
    let message =
      msg !== undefined ? msg : `${COMMIT_PREFIX} Synced file(s) with ${GITHUB_REPOSITORY}`;
    if (COMMIT_BODY) {
      message += `\n\n${COMMIT_BODY}`;
    }
  ...
  async getLastCommitMsg(path) {
    return execCmd(`git log -1 --pretty='%s' ${path}`, this.workingDir);
  }
  ...
  async createOrUpdatePr(changedFiles) {
    const body = dedent(`
      ...
      }](https://github.com/${GITHUB_REPOSITORY}/actions/runs/${process.env.GITHUB_RUN_ID || 0})
		`);
